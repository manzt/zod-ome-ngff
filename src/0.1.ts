...
export const StrictImageSchema = z
  .object({
    multiscales: Multiscales.refine((data): data is Required<z.infer<typeof Multiscales>[number]>[] => {
      return data.every((multiscale) => {
        return "name" in multiscale && "version" in multiscale && "metadata" in multiscale;
      });
    }),
    omero: Omero.optional(),
  })
